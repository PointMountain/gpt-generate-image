...
export function shouldUseOpenAIDevProxy(baseURL: string, hostname = readBrowserHostname()) {
  return isLocalBrowserHost(hostname) && normalizeOpenAIBaseURL(baseURL) !== DEFAULT_OPENAI_BASE_URL;
}

export function createOpenAIDevProxyFetch(baseURL: string): typeof fetch {
  const normalizedBaseURL = normalizeOpenAIBaseURL(baseURL);
...
export function resolveOpenAIModelsRequestTarget(baseURL: string, hostname = readBrowserHostname()) {
  const validation = validateOpenAIBaseURL(baseURL);
  if (!validation.ok) {
    return validation;
  }

  if (shouldUseOpenAIDevProxy(validation.normalizedBaseURL, hostname)) {
    return {
      ok: true as const,
...
export function resolveOpenAIProviderTransport(baseURL: string, hostname = readBrowserHostname()) {
  const validation = validateOpenAIBaseURL(baseURL);
  if (!validation.ok) {
    return validation;
  }
  ...
  if (shouldUseOpenAIDevProxy(validation.normalizedBaseURL, hostname)) {
    return {
      ...
      normalizedBaseURL: validation.normalizedBaseURL,
      baseURL: undefined,
      fetch: createOpenAIDevProxyFetch(validation.normalizedBaseURL),
    };
  }
