...
describe('image-path-input', () => {
  ...
  it('rejects oversized images before reading them into memory', async () => {
    ...
    await truncate(filePath, 50 * 1024 * 1024);

    await expect(readImageInputFromPath(filePath)).rejects.toThrow(/达到或超过 50MB/);
  });
  ...
  it('rejects oversized reference batches before loading all files', async () => {
    ...
    const files = await Promise.all(Array.from({ length: 3 }).map(async (_, index) => {
      const filePath = join(dir, `reference-${index}.png`);
      await writeFile(filePath, '');
      await truncate(filePath, 31 * 1024 * 1024);
      return filePath;
    }));

    await expect(readReferenceImagesFromPaths(files)).rejects.toThrow(/总大小超过 90MB/);
  });
});
